...
void WDOG_Enable(bool enable)
{
  ...
  BITBAND_Peripheral(&(WDOG->CTRL), _WDOG_CTRL_EN_SHIFT, (unsigned int)enable);//位操作。把enable放在寄存器
//看门狗控制寄存器的 _WDOG_CTRL_EN_SHIFT位上。
}
...
void WDOG_Feed(void)
{
  ...
  WDOG->CMD = WDOG_CMD_CLEAR;//WDOG_CMD_CLEAR =1。CMD寄存器的最低位是clear位，手册上将
//通过对cmd寄存器中的clear位写1来清除看门狗计数器
}
