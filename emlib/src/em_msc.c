...
msc_Return_TypeDef MscLoadAddress(uint32_t* address)
{
  ...
  MSC->WRITECMD = MSC_WRITECMD_LADDRIM;//加载地址并发送命令
...
msc_Return_TypeDef MSC_ErasePage(uint32_t *startAddress)
{
  ...
  MSC->WRITECTRL |= MSC_WRITECTRL_WREN;//使能写MSC

  /* Load address */
  MSC->ADDRB    = (uint32_t) startAddress;//把地址加载到MSC
  MSC->WRITECMD = MSC_WRITECMD_LADDRIM;

  /* Check for invalid address */
  if (MSC->STATUS & MSC_STATUS_INVADDR)//经检测地址是有效了
  {
  ...
  /* Check for write protected page *///当状态STATUS的lock位是1，表示上次擦除和写操作被终止
  if (MSC->STATUS & MSC_STATUS_LOCKED)//MSC_STATUS_LOCKED访问锁
  {
    /* Disable writing to the MSC */
    MSC->WRITECTRL &= ~MSC_WRITECTRL_WREN;
    return mscReturnLocked;
