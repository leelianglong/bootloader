...
*******************************************************************************/
void InsideFLASH_CalcPageSize(void)
{
  uint8_t family = *(uint8_t*)0x0FE081FE;

  if ( ( family == 71 ) || ( family == 73 ) )
  {
    InsideflashPageSize = 512;                /* Gecko and Tiny, 'G' or 'I' */
  }
  else if ( family == 72 )
  {
    InsideflashPageSize = 4096;               /* Giant, 'H' */
  }
  else /* if ( family == 74 ) */
  {
    InsideflashPageSize = 2048;               /* Leopard, 'J' */
  }
 ...
 *****************************************************************************/
__ramfunc void InsideFLASH_writeWord(uint32_t adr, uint32_t data)
{
 ...
 *****************************************************************************/
__ramfunc void InsideFLASH_writeBlock(void *block_start,
 ...
 *****************************************************************************/
__ramfunc void InsideFLASH_eraseOneBlock(uint32_t blockStart)
{
  ...
  for (ptr = (uint32_t *) blockStart; ptr < (uint32_t *)(blockStart + InsideflashPageSize); ptr++)
    acc &= *ptr;
