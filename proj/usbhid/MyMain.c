...
void NewBleFwUpdate(void)
{
	...
	SysCtlDelay(8000);//这个地方如果是80000，那么程序走到这里时，BLE_ONLINE
	...
	while((BLE_ONLINE == false)  && resetBle)
	{
		...
		SysCtlDelay(800000);
		SysCtlDelay(800000);
		SysCtlDelay(800000);//硬件复位一下等待ble给响应
	}

	if(resetBle == 0 )//说明3次硬件复位都没有唤醒，直接去MCU的APP
	{
		DealError();
	}
	...
	if(calcCRC == FW_INFO.INFO.fw_crc)
	{
		...
		{
			if((bleStFlg == 1) || (bleStFlg == 2))//这里是把两种状态信息组合起来，第一种说明是APP状态，第二种在boot状态
			{
				enter = 1;
			}
		...
		if(bleInfoCount == 0) //等待很久没有获取器件信息退出
		{
			DealError();
		}
		...
		if(bleStFlg == 1)
		{
			...
			if((BLE_DevChip.BLE_Device.WORKSTA == 1))//表示App状态
			{
				...
				while(BLE_DevChip.BLE_Device.WORKSTA && countChange)
				{
					...
					//			  SysCtlDelay(50000);
					if(BLE_DevChip.BLE_Device.WORKSTA)//如果他是1说明，上面那个状态转换命令没有响应，在发送一次
					{
						AppToBoot();
						SysCtlDelay(50000);
						memcpy(BLE_DevChip.BLE_DeviceInfo, &CopyRxBuff[UART_ID_DATA + 1], sizeof(BLE_DevChip.BLE_DeviceInfo));
		...
		if((bleStFlg == 2) || (BLE_DevChip.BLE_Device.WORKSTA == 0))//如果ble本来是boot或者通过状态装换命令之后到boot都开始写数据。
		{
			bleStFlg = 0x55;
			...
			while(!BLE_Responsed  && countStart)//下面这个是为了何工调试用要一直发送开始命令  2014年6月21日13:32:29
			{
				MyBLE_Update_Start();
				countStart--;
				SysCtlDelay(800000);
			...
			if(countStart == 0) //开始发送命令没有响应,直接退到MCU的APP
			{
				DealError();
			}
...
int main(void)
{
	...
	uint8_t  deviceCheck = 0;//用来判断这是I4还是I2。
...
//读取Information block的USER data区的数据
	...
	if(FW_INFO.INFO_BUF[10] == 0x40)//判断设备类型这是I4
	{
		deviceCheck = 1;
	}
